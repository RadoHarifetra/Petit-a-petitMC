...
import { isEventPast } from "./Agenda";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Fetch Events - Prioritize upcoming events, then past events
    const eventsQuery = query(collection(db, "agenda"), orderBy("date", "asc"));
    const unsubscribeEvents = onSnapshot(eventsQuery, (snapshot) => {
      ...
      // Take top 3
      const displayEvents = [...upcoming, ...past].slice(0, 3);
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-8">
                      <div className="flex items-baseline justify-between gap-4">
                        ...
                        {isPast && (
                          <span className="px-2.5 py-1 bg-red-600/10 border border-red-500/30 text-red-400 text-[10px] font-mono font-bold uppercase tracking-wider rounded-none">
                            Terminé
                          </span>
                        )}
                      </div>
                      ...
                      >
                        {isPast ? "Voir l'événement" : "Détails de l'événement"} <ArrowRight className="w-4 h-4" />
                      </Link>
                    </div>
                  </motion.div>
                );

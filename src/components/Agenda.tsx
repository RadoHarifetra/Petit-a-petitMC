  ...
  try {
    ...
    if (year && month && day) {
      if (timeStr && /^\d{1,2}:\d{2}$/.test(timeStr.trim())) {
        ...
        // Without time specified, consider past once the day has ended (23:59:59)
    ...
    return !isNaN(fallbackDate.getTime()) && fallbackDate.getTime() < now.getTime();
  } catch {
    return false;
  }
}
...
export default function Agenda() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const upcoming = allEvents.filter(e => !isEventPast(e.date, e.time));
      const past = allEvents.filter(e => isEventPast(e.date, e.time));
      
      // Upcoming events: closest date first
      upcoming.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
      // Past events: placed at the end, sorted most recently passed first
              ...
              return (
                ...
                >
                  <div className="relative aspect-[3/4] overflow-hidden mb-6 brutal-border bg-zinc-950">
                    ...
                    {/* Status Badge (Countdown if upcoming, "Terminé" if past) */}
                    <div className="absolute top-0 right-0 z-10">
                      {isPast ? (
                        <div className="bg-zinc-900/95 text-white px-4 py-2.5 flex items-center gap-2 border-l border-b border-white/20 shadow-2xl backdrop-blur-sm">
                          <span className="w-2 h-2 rounded-full bg-red-500" />
                          <span className="text-xs font-mono font-bold uppercase tracking-wider text-white">Terminé</span>
                        </div>
                      ) : (
                    ...
                    <div className="absolute bottom-4 left-4 z-10 flex items-center gap-2">
                      ...
                      {isPast && (
                        <div className="bg-red-600 text-white px-2.5 py-1 text-[10px] font-mono font-bold uppercase tracking-wider">
                          Terminé
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <div className="flex gap-2">
                      {isPast ? (
                        <button 
                          disabled
                          className="flex-1 py-4 bg-zinc-900 text-zinc-400 font-display uppercase text-sm tracking-widest brutal-border border-zinc-700 cursor-not-allowed flex items-center justify-center gap-2 select-none"
                        >
